  ...
  return (
    <div className="min-h-screen bg-[#07111f] flex items-center justify-center p-4">
      <div className="max-w-sm w-full text-center">
        <div className={`w-12 h-12 ${isTrialExpired ? 'bg-blue-500/10' : 'bg-amber-500/10'} rounded-xl flex items-center justify-center mx-auto mb-4`}>
          {isTrialExpired
        ...
        <div className="bg-white/5 rounded-2xl border border-white/10 p-5 mb-4 text-left">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">Cómo activar tu cuenta</p>

          <div className="space-y-3">
            {/* Paso 1 */}
            <div className="flex gap-3">
              <span className="w-5 h-5 rounded-full bg-green-500/15 text-green-400 text-[11px] font-black flex items-center justify-center shrink-0 mt-0.5">1</span>
              <div>
                <p className="text-[13px] font-semibold text-slate-300">Transferí {PRECIO} a este alias</p>
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="font-mono text-sm font-black text-white bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg">{ALIAS}</span>
                  <button onClick={copyAlias} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center transition-colors">
            ...
            {/* Paso 2 */}
            <div className="flex gap-3">
              <span className="w-5 h-5 rounded-full bg-green-500/15 text-green-400 text-[11px] font-black flex items-center justify-center shrink-0 mt-0.5">2</span>
              <div>
                <p className="text-[13px] font-semibold text-slate-300">Avisanos por WhatsApp</p>
        ...
        {isTrialExpired && (
          <div className="flex items-center justify-center gap-2 mb-4 px-3 py-2.5 bg-green-500/10 rounded-xl border border-green-500/20">
            <Zap className="w-4 h-4 text-green-400 shrink-0" />
            <p className="text-[13px] font-semibold text-green-400">Plan Negocio: {PRECIO}/mes · Todo incluido</p>
          </div>
        )}

        <p className="text-xs text-muted-foreground">
          ¿Consultas? Escribinos a{' '}
          <a href={`https://wa.me/${WA_NUMBER}`} target="_blank" rel="noopener noreferrer" className="text-green-400 underline">WhatsApp</a>
        </p>
      </div>
    </div>
  )
