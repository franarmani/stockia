...
function getCbteColor(cbteTipo: number) {
  if ([1, 2, 3].includes(cbteTipo)) return 'bg-blue-500/15 text-blue-400'
  if ([6, 7, 8].includes(cbteTipo)) return 'bg-green-500/15 text-green-400'
...
export default function ComprobantesPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Filters */}
      {showFilters && (
        <div className="bg-[#0d1b2d] border border-white/10 rounded-2xl p-4 space-y-3 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="relative sm:col-span-2">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input type="text" placeholder="Buscar por cliente, CAE o nro..." value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full h-10 pl-10 pr-3 rounded-xl border border-white/10 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-muted-foreground shrink-0" />
              <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
                className="w-full h-10 px-2 rounded-xl border border-white/10 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
            <div className="flex items-center gap-2">
            ...
            {['all', 'A', 'B', 'C', 'NC'].map(t => (
              <button key={t} onClick={() => setFilterType(t)}
                className={`px-3 py-1.5 rounded-md text-xs font-medium border transition-colors ${
                  filterType === t ? 'border-primary bg-primary/10 text-primary' : 'border-white/10 text-muted-foreground hover:text-foreground'
                }`}>
            ...
            <TableBody>
              {filtered.map((inv) => (
                <TableRow key={inv.id} className={inv.voided ? 'opacity-50' : ''}>
                  ...
                  <TableCell>
                    {inv.cae ? (
                      <span className="font-mono text-[11px] text-green-400 bg-green-500/10 px-2 py-0.5 rounded">{inv.cae}</span>
                    ) : (
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      {!inv.voided && ![3, 8, 13].includes(inv.cbte_tipo) && (
                        <button onClick={() => openNCModal(inv)} className="w-7 h-7 rounded-xl flex items-center justify-center text-muted-foreground hover:text-amber-400 hover:bg-amber-500/10 transition" title="Emitir Nota de Crédito">
                          <RotateCcw className="w-3.5 h-3.5" />
                        </button>
                      )}
                      {!inv.voided && (
                        <button onClick={() => handleVoid(inv)} className="w-7 h-7 rounded-xl flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-red-500/10 transition" title="Anular">
          ...
          <div className="space-y-4">
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex justify-between bg-white/5 rounded-xl px-3 py-2">
            ...
            {selectedInvoice.voided && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 text-center">
                <p className="text-red-400 font-semibold">Comprobante ANULADO</p>
              </div>
            )}
        ...
        {ncSourceInvoice && (
          <div className="space-y-4">
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4">
              <p className="text-sm font-semibold text-amber-300 mb-2">Factura a anular:</p>
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  <span className="text-amber-400">Nro:</span>{' '}
                ...
                <div>
                  <span className="text-amber-400">Cliente:</span>{' '}
            ...
            <div>
              ...
              <input
                ...
                className="w-full h-10 px-3 rounded-xl border border-white/10 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
