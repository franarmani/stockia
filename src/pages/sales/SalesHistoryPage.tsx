    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select value={sellerFilter} onChange={(e) => setSellerFilter(e.target.value)}
          className="h-8 px-2 rounded-xl border border-white/10 bg-white text-xs focus:outline-none focus:ring-2 focus:ring-primary/30">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm divide-y divide-white/8">
          {filtered.map((sale) => {
            ...
            return (
              <button key={sale.id} onClick={() => setSelectedSale(sale)}
                ...
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${sale.voided ? 'bg-red-50' : 'bg-white/5'}`}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {selectedSale.sale_items?.map((item: SaleWithItems['sale_items'][number]) => (
                <div key={item.id} className="flex items-center justify-between py-2 border-b border-white/8 last:border-0">
