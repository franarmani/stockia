  ...
  return (
    <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground">
      ...
      <div className="border-t border-sidebar-border p-3 shrink-0 space-y-2">
        {/* Subscription status (only desktop non-collapsed) */}
        {!collapsed && business && (() => {
          const dateState = business.trial_ends_at
            ? getSubscriptionDateState(business.trial_ends_at)
            : null
          const isExpired = dateState?.isExpired ?? false
          const daysLeft = dateState?.daysRemaining ?? 0
          const progress = dateState ? Math.min(100, (dateState.daysElapsed / dateState.totalDays) * 100) : 0
          const progressColor = isExpired ? 'bg-red-500' : daysLeft <= 7 ? 'bg-amber-500' : 'bg-primary'

          return (
            <div className="p-3 rounded-2xl bg-white/5 border border-white/10 relative overflow-hidden group/sub">
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-2">
                  <span className={`w-2 h-2 rounded-full animate-pulse ${isExpired ? 'bg-red-500' : 'bg-green-500'}`} />
                ...
                <p className={cn(
                  ...
                  isExpired ? "text-red-400" : business.subscription_status === 'trial' ? "text-amber-400" : "text-white"
                )}>
                ...
                {business.trial_ends_at && dateState && (
                  <p className="text-[9px] text-white/20 mt-1">
                    {formatShortDate(dateState.expirationDate)}
                  </p>
                )}

                {(isExpired || business.subscription_status === 'trial') && (
                  <button
                    ...
                    className="mt-3 w-full py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-[10px] font-black uppercase tracking-wider transition-all shadow-lg shadow-amber-500/20"
                  >
                    {isExpired ? 'Informar pago' : 'Pagar Ahora'}
                  </button>
                )}
              </div>
            </div>
        ...
        <button
          ...
          className={cn(
            "w-full flex items-center transition-all px-3 py-2.5 rounded-xl hover:bg-white/5 group",
            collapsed ? "justify-center" : "gap-3"
          )}
        >
