      ...
      <div className="space-y-5">
            ...
            <div className="flex gap-2">
              {([['all','Todos'],['active','Activos'],['inactive','Inactivos']] as const).map(([val, label]) => (
                <button key={val} onClick={() => setFilterActive(val)}
                  className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition ${filterActive === val ? 'bg-primary text-white border-primary' : 'border-white/10 text-muted-foreground hover:border-primary/40'}`}>
                  {label}
                </button>
              ))}
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {columns.map((col, i) => (
              <label key={col.field} className={`flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs cursor-pointer transition ${col.enabled ? 'bg-primary/5 text-primary font-medium' : 'bg-white/5 text-muted-foreground hover:bg-slate-100'}`}>
