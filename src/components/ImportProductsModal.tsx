          ...
          {STEPS.map((s, i) => (
            <div key={i} className="flex items-center flex-1">
              <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold shrink-0 transition-colors ${
                i < step ? 'bg-primary text-white' : i === step ? 'bg-primary/10 text-primary border-2 border-primary' : 'bg-white/5 text-slate-500'
              ...
              {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-1 ${i < step ? 'bg-primary' : 'bg-white/10'}`} />}
            </div>
          ))}
        </div>
          ...
          <div className="space-y-4">
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-colors ${
                dragging ? 'border-primary bg-primary/5' : 'border-white/10 hover:border-primary/50 hover:bg-white/5'
              }`}
            >
          ...
          <div className="space-y-4">
            {parsed.sheetNames.length > 1 && (
              <div>
                ...
                <select value={selectedSheet} onChange={e => handleSheetChange(e.target.value)}
                  className="w-full h-9 px-3 rounded-xl border border-white/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
            ...
            <div>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {mappings.map((m, i) => (
                  <div key={i} className={`flex items-center gap-3 p-2.5 rounded-xl border ${m.field ? 'border-green-500/20 bg-green-500/100/5' : 'border-white/10'}`}>
                    ...
                    <select
                      ...
                      className="w-48 h-9 px-2 rounded-xl border border-white/10 text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 shrink-0"
                    >
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${validRows.length > 0 ? 'bg-green-500/15 text-green-400' : 'bg-white/5 text-slate-500'}`}>
                <CheckCircle2 className="w-3.5 h-3.5" /> {validRows.length} válidas
              </div>
              {errorRows.length > 0 && (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-red-500/15 text-red-400">
            ...
            {/* Error summary */}
            {errorRows.length > 0 && (
              <div className="bg-red-500/10 rounded-xl p-3 space-y-1 max-h-32 overflow-y-auto">
                {errorRows.slice(0, 8).flatMap(r => r.errors).map((e, i) => (
                  <p key={i} className="text-xs text-red-400">
                    <span className="font-semibold">Fila {e.rowIndex}:</span> {e.message}
                  </p>
                ))}
                {errorRows.length > 8 && <p className="text-xs text-red-400">… y {errorRows.length - 8} más</p>}
              </div>
            )}
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {mappedRows.slice(0, 30).map((row) => {
                    ...
                    return (
                      <tr key={row.rowIndex} className={hasErr ? 'bg-red-500/10' : ''}>
                        <td className="px-2 py-1.5 text-muted-foreground">{row.rowIndex}</td>
                        <td className={`px-2 py-1.5 font-medium ${errFields.has('name') ? 'text-red-400' : ''}`}>{row.mapped.name || <span className="text-red-400">—</span>}</td>
                        <td className={`px-2 py-1.5 text-right ${errFields.has('sale_price') ? 'text-red-400' : ''}`}>{row.mapped.sale_price ?? <span className="text-red-400">—</span>}</td>
                        ...
                        <td className="px-2 py-1.5">
                          {hasErr
                            ? <span title={row.errors.map(e => e.message).join('; ')} className="text-red-400 cursor-help">❌</span>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {([['upsert','Crear y actualizar (recomendado)'],['create','Solo crear nuevos'],['update','Solo actualizar existentes']] as const).map(([val, label]) => (
                    <label key={val} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer text-xs transition ${mode === val ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-white/5'}`}>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {([['barcode','Código de barras'],['name','Nombre del producto']] as const).map(([val, label]) => (
                    <label key={val} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer text-xs transition ${matchBy === val ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-white/5'}`}>
          ...
          <div className="space-y-5">
            ...
            {importing && (
              <div className="space-y-3 text-center">
                ...
                <div className="w-full bg-white/5 rounded-full h-2"><div className="bg-primary h-2 rounded-full transition-all" style={{ width: `${progress}%` }} /></div>
            ...
            {result && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    { icon: '✅', label: 'Creados', val: result.created, color: 'green', bg: 'bg-green-500/10', text: 'text-green-400' },
                    { icon: '🔁', label: 'Actualizados', val: result.updated, color: 'blue', bg: 'bg-blue-500/10', text: 'text-blue-400' },
                    { icon: '⏭️', label: 'Omitidos', val: result.skipped, color: 'slate', bg: 'bg-white/5', text: 'text-slate-400' },
                    { icon: '❌', label: 'Errores', val: result.errors.length, color: 'red', bg: 'bg-red-500/10', text: 'text-red-400' },
                  ].map(s => (
                ...
                {result.errors.length > 0 && (
                  <div className="bg-red-500/10 rounded-xl p-3 space-y-1 max-h-32 overflow-y-auto">
                    {result.errors.slice(0, 6).map((e, i) => (
                      <p key={i} className="text-xs text-red-400"><span className="font-semibold">Fila {e.rowIndex}:</span> {e.message}</p>
                    ))}
                  </div>
                )}
                <div className="flex gap-2">
