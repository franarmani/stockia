...
import {
  ...
  CreditCard, ImageIcon, ThumbsUp, ThumbsDown, Bell,
} from 'lucide-react'
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<'businesses' | 'payments' | 'broadcast'>('businesses')
  const [broadcastTitle, setBroadcastTitle] = useState('')
  const [broadcastMsg, setBroadcastMsg] = useState('')
  const [broadcastUrl, setBroadcastUrl] = useState('')
  const [broadcastSeverity, setBroadcastSeverity] = useState<'info' | 'warn' | 'danger'>('info')
  ...
  async function sendBroadcast() {
    ...
    const { data, error } = await supabase.rpc('broadcast_notification', {
      ...
      p_action_url: broadcastUrl.trim() || null,
      p_severity: broadcastSeverity,
    })
    setBroadcasting(false)
    if (error) {
      toast.error('Error: ' + error.message)
    } else {
      toast.success(`✅ Notificación enviada a ${data} negocio(s)`)
      setBroadcastTitle('')
      setBroadcastMsg('')
      setBroadcastUrl('')
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 h-9 rounded-xl text-sm font-semibold transition-colors ${tab === 'broadcast' ? 'gradient-primary text-white' : 'glass-card text-white/50 hover:text-white'}`}
          >
        ...
        {tab === 'broadcast' ? (
          ...
          <div className="glass-card p-6 max-w-lg">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="text-[15px] font-bold text-white">Enviar notificación a todos</h2>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: 🎵 ¡Nueva función! Música para tu negocio"
              ...
              <div>
                <label className="text-[11px] font-semibold text-white/50 uppercase tracking-wider block mb-1.5">Mensaje *</label>
                <textarea
                  value={broadcastMsg}
                  onChange={e => setBroadcastMsg(e.target.value)}
                  placeholder="Descripción de la novedad..."
                  rows={3}
                  className="w-full bg-white/5 border border-white/15 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/25 focus:outline-none focus:border-primary/50 transition resize-none"
                />
              </div>

              <div>
                <label className="text-[11px] font-semibold text-white/50 uppercase tracking-wider block mb-1.5">URL de acción (opcional)</label>
                <input
                  type="text"
                  value={broadcastUrl}
              ...
              <div>
                <label className="text-[11px] font-semibold text-white/50 uppercase tracking-wider block mb-1.5">Severidad</label>
                <div className="flex gap-2">
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition ${
                        broadcastSeverity === s
                          ? s === 'info' ? 'bg-blue-500/20 border-blue-500/40 text-blue-300'
                            : s === 'warn' ? 'bg-amber-500/20 border-amber-500/40 text-amber-300'
                            : 'bg-red-500/20 border-red-500/40 text-red-300'
                          : 'bg-white/5 border-white/10 text-white/40 hover:text-white/70'
                      }`}
                    >
                      {s === 'info' ? '🔵 Info' : s === 'warn' ? '🟡 Aviso' : '🔴 Urgente'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                disabled={!broadcastTitle.trim() || !broadcastMsg.trim() || broadcasting}
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-bold bg-primary text-white hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed transition"
              >
                {broadcasting ? (
                ...
                {broadcasting ? 'Enviando...' : 'Enviar a todos los negocios'}
              </button>

              <p className="text-[11px] text-white/25 text-center">Solo se envía a negocios con estado activo o trial</p>
            </div>
          </div>
        ) : tab === 'payments' ? (
