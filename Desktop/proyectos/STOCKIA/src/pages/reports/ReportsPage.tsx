...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('reports-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'sales', filter: `business_id=eq.${profile.business_id}` }, () => fetchAll())
      .subscribe()
  ...
  const totalSales = activeSales.reduce((s: number, sale: any) => s + sale.total, 0)
  const totalCost = activeSales.reduce((s: number, sale: any) => {
  ...
  const paymentBreakdown = activeSales.reduce((acc: Record<string, number>, s: any) => {
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground">{activeSales.length} ventas en el período</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <p className="text-xl font-bold">{new Set(activeSales.filter((s: any) => s.customer_id).map((s: any) => s.customer_id)).size}</p>
        </div>
      </div>
