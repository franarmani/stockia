...
import {
  ...
  MessageCircle, Truck, Sparkles, UserPlus,
} from 'lucide-react'
...
export default function POSPage() {
  ...
  // Sales tip banner
  const [showSalesTip, setShowSalesTip] = useState(true)
  ...
  async function handleSaveQuickCustomer() {
    if (!quickCustName.trim() || !profile?.business_id) return
    setSavingQuickCust(true)
    try {
      const { data, error } = await supabase
        .from('customers')
        .insert({ business_id: profile.business_id, name: quickCustName.trim(), phone: quickCustPhone.trim() || null, balance: 0 })
        .select()
        .single()
      ...
      toast.success(`Cliente "${newCust.name}" guardado y seleccionado`)
    } finally {
    ...
    <div className="animate-fade-in flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 min-w-0">
          ...
          {/* Sales tip banner */}
          {showSalesTip && (
            <div className="mx-3 sm:mx-4 mt-3 relative">
              <div className="rounded-2xl bg-linear-to-r from-[#25D366]/10 via-primary/8 to-blue-500/10 border border-[#25D366]/20 backdrop-blur-sm animate-fade-in overflow-hidden">
                {/* Close button */}
                <button
                  onClick={() => setShowSalesTip(false)}
                  className="absolute top-2 right-2 text-white/20 hover:text-white/50 transition p-1 z-10"
                >
                ...
                <div className="px-4 py-3">
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-xl bg-[#25D366]/15 border border-[#25D366]/25 flex items-center justify-center shrink-0 mt-0.5">
                      <Sparkles className="w-4 h-4 text-[#25D366]" />
                    </div>
                    <div className="flex-1 min-w-0 pr-4">
                      <p className="text-[12px] font-semibold text-white/80 leading-relaxed">
                        <span className="text-[#25D366] font-bold">Tip de venta:</span> Recordale al cliente que hacemos
                        <span className="inline-flex items-center gap-1 mx-1 text-blue-300"><Truck className="w-3 h-3" />envíos a domicilio</span>
                        y atención por
                        <span className="inline-flex items-center gap-1 mx-1 text-[#25D366]"><MessageCircle className="w-3 h-3" />WhatsApp</span>
                        para pedidos y consultas.
                      </p>
                      <p className="text-[11px] text-white/50 mt-1.5 leading-relaxed">
                        <span className="text-white/70 font-medium">Fidelizá a tu cliente con un canal directo.</span> Registrá su nombre, apellido y número de WhatsApp para enviarle novedades, ofertas y comprobantes al instante.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-4 py-2.5 border-t border-white/5 bg-white/[0.02]">
                  <button
                    onClick={() => { setShowCustomerModal(true); setShowQuickAddCustomer(true) }}
                    className="flex items-center gap-1.5 h-7 px-3 rounded-lg bg-[#25D366]/15 border border-[#25D366]/25 text-[#25D366] text-[11px] font-semibold hover:bg-[#25D366]/25 active:scale-95 transition"
                  >
                    <UserPlus className="w-3 h-3" /> Agregar cliente
                  </button>
                  <button
                    onClick={() => window.open('/whatsapp', '_blank')}
                    className="flex items-center gap-1.5 h-7 px-3 rounded-lg bg-white/5 border border-white/10 text-white/50 text-[11px] font-semibold hover:bg-white/10 hover:text-white/70 active:scale-95 transition"
                  >
                    <MessageCircle className="w-3 h-3" /> Ir a WhatsApp
                  </button>
                </div>
              </div>
            </div>
      ...
      <Modal open={showCustomerModal} onClose={() => setShowCustomerModal(false)} title="Seleccionar cliente" size="sm">
        ...
        {!showQuickAddCustomer ? (
          <button
            ...
            className="w-full flex items-center gap-2.5 p-3 rounded-xl border border-dashed border-primary/30 bg-primary/5 hover:bg-primary/10 text-sm font-medium text-primary transition mb-3"
          >
          ...
          <div className="p-3 rounded-xl border border-primary/30 bg-primary/5 space-y-2 mb-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-primary">Nuevo cliente</span>
              <button onClick={() => { setShowQuickAddCustomer(false); setQuickCustName(''); setQuickCustPhone('') }} className="text-white/30 hover:text-white/60">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
            <input type="text" placeholder="Nombre *" value={quickCustName} onChange={(e) => setQuickCustName(e.target.value)} autoFocus
              className="w-full h-9 px-3 rounded-lg border border-white/10 bg-white/5 text-white placeholder:text-white/30 text-sm focus:outline-none focus:ring-1 focus:ring-primary/40" />
            <input type="tel" placeholder="Teléfono (opc.)" value={quickCustPhone} onChange={(e) => setQuickCustPhone(e.target.value)}
              className="w-full h-9 px-3 rounded-lg border border-white/10 bg-white/5 text-white placeholder:text-white/30 text-sm focus:outline-none focus:ring-1 focus:ring-primary/40" />
            <button
              disabled={!quickCustName.trim() || savingQuickCust}
              onClick={handleSaveQuickCustomer}
              className="w-full flex items-center justify-center gap-2 h-9 rounded-lg bg-primary hover:bg-primary/80 disabled:opacity-40 text-white text-xs font-semibold transition"
            >
              {savingQuickCust ? <span className="animate-spin">⏳</span> : <Check className="w-3.5 h-3.5" />}
