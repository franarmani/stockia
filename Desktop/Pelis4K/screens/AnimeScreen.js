...
const AnimeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    filterAnimes();
  }, [searchQuery, animes, selectedGenre]);
  ...
  const loadAnimes = async () => {
    try {
      ...
      setAnimes(reversedData);
      setFilteredAnimes(reversedData);
    } catch (error) {
  ...
  const handleCast = async (item) => {
    try {
      ...
      console.log('🎌 Preparando para transmitir anime:', item.title || item.name);
      ...
      <View style={styles.searchSection}>
        ...
        <View style={styles.resultsInfo}>
          <Text style={styles.resultsText}>
            {searchQuery ? `${filteredAnimes.length} resultados` : `${animes.length} animes disponibles`}
          </Text>
        </View>
      </View>
  ...
  const renderEmptySearch = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyTitle}>No se encontraron animes</Text>
      ...
      <View style={styles.header}>
          <View style={styles.headerContent}>
            <View style={styles.titleContainer}>
              <Ionicons name="radio" size={28} color="#9D50BB" />
              <Text style={styles.headerTitle}>Anime</Text>
            </View>
            <View style={styles.headerBadge}>
              <Text style={styles.headerBadgeText}>SUB</Text>
            </View>
          </View>
        </View>
        <View style={styles.centered}>
          ...
          <Text style={styles.loadingTitle}>Cargando anime</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        contentContainerStyle={[
          ...
          filteredAnimes.length === 0 && styles.emptyListContainer
        ]}
        ListEmptyComponent={searchQuery ? renderEmptySearch : null}
        key={`anime-flatlist-${viewMode}`}
      ...
      <ChromecastModal
        ...
        onStartCasting={(castData) => {
          console.log('✅ Iniciada transmisión de anime:', castData);
          setTimeout(() => {
            setShowChromecastModal(false);
          }, 2000);
        }}
